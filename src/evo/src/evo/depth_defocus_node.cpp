...
// Build a kindr transformation from a geometry_msgs transform (replaces
// minkindr_conversions / tf::transformTFToKindr).
Transformation msgToKindr(const geometry_msgs::msg::Transform& m) {
    const Quaternion q(m.rotation.w, m.rotation.x, m.rotation.y, m.rotation.z);
    const Eigen::Vector3d t(m.translation.x, m.translation.y, m.translation.z);
...
void DepthFromDefocusNode::setup(
    ...
    state_ = IDLE;
    dvs_cam_ = cam;
    pc_ = PointCloud::Ptr(new PointCloud);
    pc_global_ = PointCloud::Ptr(new PointCloud);
    ...
    regular_frame_id_ =
        rpg_common_ros::param<std::string>(nh_, "dvs_frame_id", "dvs");
    bootstrap_frame_id_ = rpg_common_ros::param<std::string>(
        nh_, "dvs_bootstrap_frame_id", regular_frame_id_);
    ...
    pub_pc_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
        "~/pointcloud", rclcpp::QoS(1));
    pc_->header.frame_id = world_frame_id_;

    pub_pc_global_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
        "~/pointcloud_global", rclcpp::QoS(1));
    pc_global_->header.frame_id = world_frame_id_;

    pub_voxel_grid_ = node_->create_publisher<dvs_slam_msgs::msg::VoxelGrid>(
        "~/voxel_grid", rclcpp::QoS(1));

    pub_depth_map_ = node_->create_publisher<sensor_msgs::msg::Image>(
        "~/depthmap", rclcpp::QoS(1));
...
void DepthFromDefocusNode::setCopilot(bool regular) {
    frame_id_ = regular ? regular_frame_id_ : bootstrap_frame_id_;
...
void DepthFromDefocusNode::onTrackedPose(const rclcpp::Time& stamp) {
    ...
    const int64_t s = stamp.nanoseconds();

    while (newest_tracked_event_ + 1 < event_queue_.size() &&
           event_queue_[newest_tracked_event_ + 1].ts.toNSec() < s)
...
void DepthFromDefocusNode::resetMapper() {
    event_queue_.clear();
    // NOTE: the original cleared its private tf buffer here; in the merged node
    // the tf2::BufferCore is shared with the tracker/bootstrapper and must not
    // be wiped on mapper reset.
    pc_->clear();
    pc_global_->clear();
    setupVoxelGrid();
...
void DepthFromDefocusNode::processEventQueue(bool reset) {
    ...
    while (current_event_ + (skip_batches + 1) * frame_size <
        ...
        for (size_t i = 0; i != frame_size; ++i) {
            const evo::Event& e = event_queue_[current_event_];
            Eigen::Vector4f p;
...
void DepthFromDefocusNode::resetVoxelGrid() {
    ...
    evo::EventTime last_stamp = event_queue_[newest_tracked_event_].ts;
    ...
    LOG(INFO) << "Setting KF at time: " << last_stamp.toNSec();
...
void DepthFromDefocusNode::addEvents(const std::vector<evo::Event>& events) {
    ...
    for (const evo::Event& e : events) event_queue_.push_back(e);

    // Add initial pose
    static bool sent_initial_pose = false;
    if (!sent_initial_pose) {
        geometry_msgs::msg::TransformStamped T;
        T.header.stamp = rclcpp::Time(event_queue_[0].ts.toNSec());
        T.header.frame_id = world_frame_id_;
        T.child_frame_id = frame_id_;
        T.transform.rotation.w = 1.0;
        tf_buffer_->setTransform(T, "evo_mapper_init");
...
bool DepthFromDefocusNode::getPoseAt(const evo::EventTime& t,
                                     evo_utils::geometry::Transformation& T) {
    const tf2::TimePoint tp{std::chrono::nanoseconds(t.toNSec())};
    std::string error_msg;
    if (!tf_buffer_->canTransform(world_frame_id_, frame_id_, tp, &error_msg)) {
        LOG(WARNING) << t.toNSec() << " : " << error_msg;
        return false;
    } else {
        geometry_msgs::msg::TransformStamped tr =
            tf_buffer_->lookupTransform(world_frame_id_, frame_id_, tp);

        T = msgToKindr(tr.transform);
...
void DepthFromDefocusNode::synthesizePointCloudFromVoxelGrid(
    ...
    if (type_focus_measure == 1) {
        ...
        // Maximum patch focus along optical ray
...
void DepthFromDefocusNode::publishDepthmap(const cv::Mat& depth,
                                           const cv::Mat& mask) {
    if (pub_depth_map_->get_subscription_count() == 0) return;
...
void DepthFromDefocusNode::publishVoxelGrid(
    ...
    if (pub_voxel_grid_->get_subscription_count() > 0) {
        ...
        geometry_msgs::msg::Pose T_w_ref_msg;
        dvs_slam_msgs::msg::VoxelGrid voxel_grid_msg;
...
void DepthFromDefocusNode::publishGlobalMap() {
    ...
    if (pub_pc_global_->get_subscription_count() > 0 && skipped >= skip_first &&
        ...
        *pc_global_ += *pc_;
        ...
        if (radius_search > 0) {
            ...
            pc_global_->swap(*cloud_filtered2);
        }
